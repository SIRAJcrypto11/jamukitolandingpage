...
export default function TaskFloatingAI({ workspaceId, onTaskCreated }) {
  ...
  const calculateCreditsNeeded = (prompt) => {
    // Simple task creation: 1 credit
    // Complex task with multiple subtasks: 3 credits
    // Task with research/internet: 5 credits
    const lowerPrompt = prompt.toLowerCase();

    if (/(cari|riset|referensi|analisis mendalam|internet)/.test(lowerPrompt)) {
      return 5;
    }
    if (/(banyak|beberapa|multiple|subtask|sub-task)/.test(lowerPrompt)) {
      return 3;
    }
    return 1;
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const prompt = `Kamu adalah AI assistant untuk manajemen tugas. User berada di halaman Tasks.
...
Tugas kamu:
1. Jika user meminta untuk membuat tugas, buat dengan due_date yang akurat
2. Jika user bertanya tentang tugas, berikan saran produktivitas
3. Jika user butuh prioritas tugas, bantu analisis
4. PENTING: Gunakan format ISO 8601 untuk due_date: YYYY-MM-DDTHH:mm:ss+07:00

Permintaan user: "${input}"

Respons dalam format JSON:
{
  "action": "create_task" | "chat",
  "tasks": [{"title": "...", "content": "...", "priority": "low|medium|high|urgent", "due_date": "..."}],
  "message": "..."
}`;
      ...
      if (response.action === 'create_task' && response.tasks) {
        if (!workspaceId) {
          throw new Error('Workspace ID tidak ditemukan');
        }
      ...
      setMessages((prev) => [...prev, {
        role: 'assistant',
        content: `${response.message || 'Tugas berhasil dibuat!'} (Digunakan ${creditsNeeded} kredit)`
      }]);
      ...
      <AnimatePresence>
        ...
        <motion.div
          ...
          className="md:bottom-6 md:right-6">
            ...
            size="lg" className="bg-gradient-to-r text-primary-foreground mb-20 pr-8 pl-8 text-sm font-medium inline-flex items-center justify-center gap-2 whitespace-nowrap ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 hover:bg-primary/90 h-12 w-12 md:h-14 md:w-14 rounded-full shadow-2xl from-blue-500 to-cyan-600 hover:from-blue-600 hover:to-cyan-700 hover:scale-110 transition-transform">
          ...
          className="w-[calc(100vw-2rem)] max-w-96 h-[500px] md:bottom-6 md:right-6 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Bantu kelola tugas Anda</p>
                  </div>
                </div>
            ...
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  ...
                  <ul className="text-xs mt-2 space-y-1">
                    <li>• Buat tugas otomatis (1-5 kredit)</li>
                    <li>• Atur prioritas tugas</li>
                    <li>• Saran produktivitas</li>
                  </ul>
                </div>
            }
              {messages.map((msg, idx) =>
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2 ${msg.role === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100'}`}>
